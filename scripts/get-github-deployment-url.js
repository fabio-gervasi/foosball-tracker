...
async function main() {
  ...
  if (!commitSha || !repoOwner || !repoName) {
    console.error(
      '❌ Usage: node get-github-deployment-url.js <commit-sha> <repo-owner> <repo-name> [github-token]'
    );
    console.error(
      '   Example: node get-github-deployment-url.js abc123def fabio-gervasi foosball-tracker'
    );
    console.error(
      '   GitHub token can be provided as argument or GITHUB_TOKEN environment variable'
    );
    process.exit(1);
  }
