...
export function MatchConfirmation({
  ...
  const getMatchSummary = () => {
    ...
    if (match.matchType === '1v1') {
      ...
      const winningPlayer1Name =
        winningTeam?.player1?.name || winningTeam?.player1?.id || 'Unknown';
      const winningPlayer2Name =
        winningTeam?.player2?.name || winningTeam?.player2?.id || 'Unknown';
    ...
    <div className='p-4 space-y-6 pb-8'>
      ...
      <div className='text-center py-6'>
        ...
        <p className='text-gray-600'>
          Thank you for uploading the match details.
          <br />
      ...
      <div className='bg-white rounded-lg border border-gray-200 overflow-hidden'>
        ...
        <div className='divide-y divide-gray-200'>
          ...
          <div className='grid grid-cols-2 divide-x divide-gray-200'>
            ...
            <div className='px-4 py-3 text-sm text-gray-900'>
              {formatDate(matchResult.match.date)}
            </div>
          </div>
          ...
          <div className='grid grid-cols-2 divide-x divide-gray-200'>
            ...
            <div className='px-4 py-3 text-sm text-gray-900'>
              ...
              {matchResult.match.isSweep && (
                <span className='text-green-600 ml-2 font-medium'>2-0 Sweep!</span>
              )}
            </div>
          </div>
          ...
          <div className='grid grid-cols-2 divide-x divide-gray-200'>
            ...
            <div className='px-4 py-3 text-sm text-gray-900'>
              {matchSummary.losingTeam} (score: {matchSummary.score.split('-').reverse().join('-')})
            </div>
          </div>
        </div>
      </div>
      ...
      <div className='bg-white rounded-lg border border-gray-200 overflow-hidden'>
        <div className='grid grid-cols-12 divide-x divide-gray-200'>
          <div className='col-span-7 px-4 py-3 bg-gray-50 text-sm font-medium text-gray-700'>
            Players
          </div>
          <div className='col-span-2 px-2 py-3 bg-gray-50 text-sm font-medium text-gray-700 text-center'>
            Change
          </div>
          <div className='col-span-3 px-3 py-3 bg-gray-50 text-sm font-medium text-gray-700 text-center'>
            New Rating
          </div>
        </div>
          ...
          {allPlayers.map((player, index) => {
            ...
            return (
              <div key={player.id} className='grid grid-cols-12 divide-x divide-gray-200'>
                ...
                <div
                  className={`col-span-2 px-2 py-3 text-sm text-center font-medium whitespace-nowrap ${
                ...
                <div className='col-span-3 px-3 py-3 text-sm text-gray-900 text-center font-medium'>
                  {newRating}
                </div>
              </div>
            );
      ...
      <div className='bg-white rounded-lg border border-gray-200 p-4'>
        <p className='text-sm text-gray-600 text-center mb-4'>
          If you need to make any corrections, you can delete the match by clicking the button
          below:
        </p>
