...
import { User, Users, Code, LogOut, Shield, Camera, Upload, X, Loader2, Lock, Key, ChevronDown, Plus } from 'lucide-react';
...
export function Profile({ user, group, accessToken, onUpdateProfile, onDataChange, onGroupChanged }: ProfileProps) {
  ...
  const uploadAvatar = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch(`https://${projectId}.supabase.co/functions/v1/make-server-171cbf6f/profile/avatar`, {
        method: 'POST',
        headers: {
  ...
  const winRate = user.wins + user.losses > 0 ? (user.wins / (user.wins + user.losses) * 100).toFixed(1) : '0';
              ...
              >
                ...
                <div className="flex items-center space-x-2">
                  ...
                  {isSwitchingGroup ? (
                    ...
                    <ChevronDown className={`w-4 h-4 text-gray-400 transition-transform ${showGroupSelector ? 'rotate-180' : ''}`} />
                  )}
                </div>
              </button>
                  ...
                  {isLoadingGroups ? (
                    ...
                    userGroups.map((userGroup) => (
                      ...
                      >
                        ...
                        <div className="flex items-center space-x-2">
                          <span className="text-xs text-gray-600 tracking-wider">{userGroup.code}</span>
                          {userGroup.isCurrent && (
          ...
          {group && (
            <div className="text-center mt-4 pt-4 border-t border-gray-200">
              ...
              <p className="text-xs text-gray-500 mt-1">
                Warning: You'll lose all match history
              </p>
            </div>
          )}
        </div>
          ...
          <div className="bg-white rounded-lg max-w-md w-full">
            ...
            <form onSubmit={handleCreateGroup} className="p-4 space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">Choose a name for your foosball group (min. 3 characters)</p>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={(e) => setJoinGroupForm(prev => ({ ...prev, groupCode: e.target.value.toUpperCase() }))}
