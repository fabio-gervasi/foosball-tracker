...
function EloChart({ data }: { data: Array<{ date: string; elo: number; formattedDate: string }> }) {
  ...
  const scaleY = (elo: number) => chartHeight - padding.bottom - ((elo - minElo) / eloRange) * chartInnerHeight;
...
export function Statistics({ user, matches, group }: StatisticsProps) {
    ...
    return matches.filter(match => {
      ...
      if (match.matchType === '1v1' || !match.matchType) {
        ...
        if (match.player1?.id && match.player2?.id) {
          ...
          // Legacy format
          isUserInMatch = match.player1Email === userIdentifier || match.player2Email === userIdentifier ||
  ...
  const totalEloChange = userMatches.reduce((total, match) => {
    const userEloChange = match.eloChanges?.[user.email]?.change || match.eloChanges?.[userIdentifier]?.change || match.eloChanges?.[user.id]?.change || 0;
    return total + userEloChange;
  }, 0);

  const averageEloChange = userMatches.length > 0 ? (totalEloChange / userMatches.length).toFixed(1) : '0';
  ...
  const eloTimelineData = useMemo(() => {
    ...
    sortedMatches.forEach(match => {
      const userEloData = match.eloChanges?.[user.email] || match.eloChanges?.[userIdentifier] || match.eloChanges?.[user.id];
    ...
    twoVsTwoMatches.forEach(match => {
      ...
      if (match.team1?.player1?.id && match.team1?.player2?.id && match.team2?.player1?.id && match.team2?.player2?.id) {
        ...
        if (isInTeam1) {
          ...
          const isPlayer1 = match.team2Player1Email === userIdentifier || match.team2Player1Email === user.email;
  ...
  const singles1v1Count = allUserMatches.filter(match => match.matchType === '1v1' || !match.matchType).length;
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 text-center">
        <div className="flex items-center justify-center mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-blue-600">Performance analytics in {group?.name || 'your group'}</p>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-cyan-600 text-sm">
                {userMatches.length} matches filtered
              </p>
            </div>
          </div>
        </div>
          ...
          <div>
            ...
            <div className="flex items-center space-x-2">
              <div className={`px-3 py-1 rounded-full text-sm font-medium ${
                ...
                {currentStreak > 0 ? `${currentStreak} ${streakType === 'W' ? 'Win' : 'Loss'}${currentStreak > 1 ? 's' : ''}` : 'No streak'}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div className="bg-white border border-teal-100 rounded-lg p-4">
            ...
            <div className="text-sm text-teal-600">
              Current {selectedMatchType === '1v1' ? '1v1' : selectedMatchType === '2v2' ? '2v2' : ''} ELO
            </div>
          </div>
          <div className="bg-white border border-teal-100 rounded-lg p-4">
            <div className={`text-2xl ${
              totalEloChange >= 0 ? 'text-green-600' : 'text-red-600'
            }`}>
          ...
          <div className="bg-white border border-teal-100 rounded-lg p-4">
            ...
            <div className="text-sm text-teal-600">Peak {selectedMatchType === '1v1' ? '1v1' : selectedMatchType === '2v2' ? '2v2' : ''} ELO</div>
          </div>
          <div className="bg-white border border-teal-100 rounded-lg p-4">
            <div className={`text-2xl ${
              parseFloat(averageEloChange) >= 0 ? 'text-green-600' : 'text-red-600'
            }`}>
              {parseFloat(averageEloChange) >= 0 ? '+' : ''}{averageEloChange}
            </div>
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-gray-600 text-sm mb-2">
                  Your win rate: {((mostFearedOpponent.wins / mostFearedOpponent.total) * 100).toFixed(1)}%
                </p>
                <p className="text-gray-700 text-xs">
                  {mostFearedOpponent.wins}W - {mostFearedOpponent.losses}L ({mostFearedOpponent.total} games)
                </p>
              </div>
            </div>
          </div>
