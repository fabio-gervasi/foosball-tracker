...
export function Leaderboard({ users, group, currentUser, accessToken }: LeaderboardProps) {
  ...
  const getRankIcon = (rank: number) => {
    switch (rank) {
      ...
      default:
        return (
          <span className='w-6 h-6 flex items-center justify-center text-gray-500 text-sm'>
            #{rank}
          </span>
        );
    }
                ...
                <div className='flex items-center space-x-4'>
                  ...
                  <div className='flex-1'>
                    ...
                    <div className='space-y-0.5'>
                      <p className='text-sm text-gray-500'>ELO: {elo}</p>
      ...
      {users.length < 5 && (
        <div className='bg-yellow-50 border border-yellow-200 rounded-lg p-4'>
          ...
          <p className='text-sm text-yellow-700 mb-2'>
            Invite more colleagues to join <strong>{group?.name}</strong> for a more competitive
            leaderboard!
          </p>
          <p className='text-xs text-yellow-600'>
            Share group code:{' '}
            <span className='font-mono bg-yellow-100 px-1 rounded'>{group?.code}</span>
          </p>
        </div>
      )}
    </div>
