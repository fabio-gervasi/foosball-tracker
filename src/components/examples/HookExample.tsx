...
export function HookExample() {
  ...
  const { currentUser, isLoggedIn, isLoading: authLoading, logout, isAdmin } = useAuth();
  ...
  // Permission checking
  const { canSubmitMatches, canViewAdminPanel, hasPermission, checkPermission } = usePermissions();

  // Match validation
  const { validateMatch, isValidEmail, rules: validationRules } = useMatchValidation();
    ...
    <div className='space-y-6 p-6'>
      <div className='bg-white rounded-lg shadow p-6'>
        ...
        <div className='mb-6'>
          ...
          <div className='bg-gray-50 p-4 rounded'>
            ...
            <p>
              <strong>Current Group:</strong> {currentGroup?.name || 'None'}
            </p>
            <p>
          ...
          <div className='bg-gray-50 p-4 rounded'>
            <div className='space-y-2'>
              <div className='flex items-center justify-between'>
                ...
                <span
                  className={`px-2 py-1 rounded text-sm ${canSubmitMatches ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}
                >
              ...
              <div className='flex items-center justify-between'>
                ...
                <span
                  className={`px-2 py-1 rounded text-sm ${canViewAdminPanel ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}
                >
              ...
              <div className='flex items-center justify-between'>
                ...
                <span
                  className={`px-2 py-1 rounded text-sm ${hasPermission('manage_users') ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}
                >
        ...
        <div className='mb-6'>
          ...
          <div className='bg-gray-50 p-4 rounded'>
            <div className='space-y-2'>
              <p>
                <strong>Min Score:</strong> {validationRules.minScore}
              </p>
              <p>
                <strong>Max Score:</strong> {validationRules.maxScore}
              </p>
              <p>
                <strong>Max Score Difference:</strong> {validationRules.maxScoreDifference}
              </p>
              <p>
                <strong>Requires Winner:</strong> {validationRules.requiresWinner ? 'Yes' : 'No'}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className='bg-white rounded-lg shadow p-6'>
        ...
        <div className='space-y-3'>
          {(['admin', 'manage_users', 'submit_matches', 'view_statistics'] as const).map(
            permission => {
