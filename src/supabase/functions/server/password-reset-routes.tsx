...
export function createPasswordResetRoutes(supabase: any) {
  ...
  app.post('/password-reset', async (c) => {
    try {
      ...
      const { data, error: resetError } = await clientSupabase.auth.resetPasswordForEmail(
        userProfile.email,
        {
          redirectTo: redirectUrl || `${Deno.env.get('SITE_URL') || 'http://localhost:3000'}/password-reset-callback`
        }
      );
      ...
      if (resetError) {
        ...
        if (resetError.message?.includes('Email rate limit exceeded')) {
          return c.json({ error: 'Too many password reset attempts. Please wait before trying again.' }, 429);
          ...
          return c.json({ error: 'Email service is not fully configured. Please contact your administrator.' }, 503);
        } else {
          return c.json({ error: resetError.message || 'Failed to send password reset email' }, 400);
        }
      }
  ...
  app.post('/update-password', async (c) => {
    try {
      ...
      const { data: sessionData, error: sessionError } = await clientSupabase.auth.getUser(accessToken);
