...
// Helper to validate user authorization for protected endpoints
export async function validateUserAuth(c: any, supabase: any) {
  console.log('=== Starting validateUserAuth ===');
  ...
  try {
    ...
    try {
      ...
      if (finalError || !user) {
        ...
        return { error: 'JWT validation failed', status: 401 };
      }
...
export async function validateAdminAuth(c: any, supabase: any) {
  ...
  return { user: authResult.user, userProfile };
}