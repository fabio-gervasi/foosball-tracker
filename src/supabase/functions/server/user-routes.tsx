  ...
  app.get('/users', async (c) => {
    try {
      ...
      if (groupUserEntries.length > 0) {
        console.log('Sample group-user entries:', groupUserEntries.slice(0, 3).map(e => ({
          key: e.key,
          value: e.value
        })));
      } else {
        console.log('No group-user entries found. This suggests the group-user relationships are missing.');
        ...
        console.log('Group data:', groupData ? {
          ...
          membersArrayLength: Array.isArray(groupData.members) ? groupData.members.length : 'not an array',
