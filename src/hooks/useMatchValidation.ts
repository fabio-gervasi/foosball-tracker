...
export const useMatchValidation = (): UseMatchValidationReturn => {
  ...
  const isValidScore = useCallback(
    (score: number): boolean => {
      return Number.isInteger(score) && score >= rules.minScore && score <= rules.maxScore;
    },
    [rules]
  );
  ...
  const validatePlayers = useCallback(
    (players: Player[]): ValidationResult => {
  ...
  const validateScore = useCallback(
    (score1: number, score2: number): ValidationResult => {
      const errors: ValidationError[] = [];
  ...
  const validateMatchType = useCallback(
    (matchType: string, players: Player[]): ValidationResult => {
      const errors: ValidationError[] = [];
  ...
  const validateMatch = useCallback(
    (matchData: MatchValidationData): ValidationResult => {
