...
export const useApiRequest = <T = any>(
  ...
  const buildHeaders = useCallback(
    (options: ApiRequestOptions): Record<string, string> => {
  ...
  const execute = useCallback(
    async (endpoint: string, options?: ApiRequestOptions): Promise<T> => {
      ...
      try {
        ...
        if (processedError.recoverable && retryCountRef.current < (mergedOptions.retries || 0)) {
    ...
    [mergeOptions, buildHeaders, buildQueryString, getRetryDelay, isLoggedIn]
  );
